"""Guarded runner for the V2.2 single-Q06 Flash real revalidation."""
...
CONFIRMATION = "I_AUTHORIZE_Q06_V2_2_FLASH_REAL_REVALIDATION"
...
def validate_execution_request(
    ...
    *,
    ...
    candidate = load_q06_v2_2_flash_real_plan() if plan is None else plan
    preflight = validate_q06_v2_2_flash_real_plan(candidate)
    ...
    if args.confirm_real_model_calls != CONFIRMATION:
        raise ValueError("exact V2.2 Flash confirmation is missing")
    if preflight.real_model_calls_allowed is not True:
        raise ValueError("V2.2 Flash plan has no separate real-call authority")
    ...
    *,
    ...
    return core.execute_q06_validation(
        ...
        or datetime.now().astimezone().strftime(
            "q06_v2_2_flash_real_revalidation_%Y%m%dT%H%M%S_%f%z"
        ),
    )
...
def main() -> int:
    ...
    result = execute_v2_2_validation(
        api_key=api_key,
        registry=registry,
        transport=None,
