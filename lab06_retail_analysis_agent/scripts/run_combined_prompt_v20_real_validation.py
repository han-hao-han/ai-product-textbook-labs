"""Run the v20 Q01/Q06 checkpoint or Q01-Q10 full validation."""
...
from src.combined_prompt_v20_real_runner import (  # noqa: E402
    execute_validation_v20_checkpoint, execute_validation_v20_full,
    validate_real_request_v20_checkpoint, validate_real_request_v20_full,
)
...
def main() -> int:
    ...
    validator = validate_real_request_v20_checkpoint if args.scope == "checkpoint" else validate_real_request_v20_full
    executor = execute_validation_v20_checkpoint if args.scope == "checkpoint" else execute_validation_v20_full
    try:
        authority = validator(
            model=args.model, response_cap=args.response_cap,
    ...
    api_key = os.environ.get("LLM_API_KEY") or _dotenv(REPOSITORY_ROOT / ".env", "LLM_API_KEY")
    ...
    registry = RetailToolRegistry(RetailToolService(build_retail_data_layers(pd.read_excel(workbook))))
    result = executor(
        api_key=api_key, registry=registry, transport_factory=None,
        authority=authority, output_parent=PROJECT_ROOT / "results" / "raw",
    )
