"""V2.3.4.2 protocol candidate with tool-routing Prompt v2."""
...
TOOL_ROUTING_PROMPT_VERSION = "1.5.6-h3-native-tool-routing-v2"
TOOL_ROUTING_PROMPT_PATH = (
    PROJECT_ROOT / "prompts" / "native_tool_agent_system_tool_routing_v2.md"
)


def project_runtime_messages_tool_routing_v2(
        ...
        if copied.get("role") == "system" and isinstance(content, str):
            if old_system not in content:
                raise DeepSeekClientError(
                    "tool_routing_prompt_v2_projection: frozen system Prompt marker missing"
                )
    ...
    if replacements != 1:
        raise DeepSeekClientError(
            "tool_routing_prompt_v2_projection: exactly one system Prompt is required"
        )
    return result


@dataclass
class NativeToolOnlineCandidateToolRoutingV2(
    NativeToolOnlineCandidateToolRoutingV1
):
    runtime_message_projector: Callable[
        ...
        init=False,
        default_factory=lambda: project_runtime_messages_tool_routing_v2,
        repr=False,
    )
