"""Single-use Q01-Q10 real validation for combined Prompt v15."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_combined_prompt_v15_full_real_validation.json"
REAL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V15_Q01_Q10_REAL_CALLS"
...
BASELINE_RESPONSE_COUNT = sum(
    EXPECTED_RESPONSES[item] for item in QUESTION_ORDER
)
RESPONSE_CAP = BASELINE_RESPONSE_COUNT + sum(REPAIR_ALLOWANCE.values())


def validate_real_request_v15_full(
    *, model: str, response_cap: int, automatic_retries: int,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v15 full authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v15 full authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def execute_validation_v15_full(
    *, api_key: str, registry: Any, transport_factory: Any | None,
    ...
    return execute_validation(
        ...
        schema_namespace="combined-prompt-v15-full",
...
__all__ = [
    ...
    "REPAIR_ALLOWANCE", "RESPONSE_CAP", "execute_validation_v15_full",
    "validate_real_request_v15_full",
]
