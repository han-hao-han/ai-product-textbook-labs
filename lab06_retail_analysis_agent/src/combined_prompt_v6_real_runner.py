"""Single-use four-question real checkpoint for combined Prompt v6."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_combined_prompt_v6_four_q_real_validation.json"
REAL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V6_FOUR_Q_REAL_CALLS"
QUESTION_ORDER = ("Q04", "Q06", "Q05", "Q07")
RESPONSE_CAP = sum(EXPECTED_RESPONSES[item] for item in QUESTION_ORDER)


def validate_real_request_v6(
    *, model: str, response_cap: int, automatic_retries: int,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v6 real authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v6 real authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def execute_validation_v6(
    *, api_key: str, registry: Any, transport_factory: Any | None,
    ...
    return execute_validation(
        ...
        candidate_type=NativeToolOnlineCandidateCombinedV6,
        prompt_version=COMBINED_PROMPT_VERSION,
        schema_namespace="combined-prompt-v6-four-q",
...
__all__ = [
    ...
    "execute_validation_v6", "validate_real_request_v6",
]
