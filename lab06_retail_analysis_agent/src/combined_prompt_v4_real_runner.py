"""Single-use failed-question real checkpoint for combined Prompt v4."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_combined_prompt_v4_failed_real_validation.json"
REAL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V4_FAILED_Q_REAL_CALLS"
QUESTION_ORDER = (
    "Q04", "Q07", "Q08", "Q09", "Q10",
    "Q02", "Q03", "Q05", "Q06",
)
...
def validate_real_request_v4(
    *, model: str, response_cap: int, automatic_retries: int,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v4 real authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v4 real authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def execute_validation_v4(
    *, api_key: str, registry: Any, transport_factory: Any | None,
    ...
    return execute_validation(
        ...
        schema_namespace="combined-prompt-v4-failed-q",
...
__all__ = [
    ...
    "execute_validation_v4", "validate_real_request_v4",
]
