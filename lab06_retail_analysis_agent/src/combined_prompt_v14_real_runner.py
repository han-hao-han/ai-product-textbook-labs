"""Single-use Q04 checkpoint with two bounded terminal corrections."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_combined_prompt_v14_q04_real_validation.json"
REAL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V14_Q04_REAL_CALLS"
QUESTION_ORDER = ("Q04",)
REPAIR_ALLOWANCE = {"Q04": 2}
RESPONSE_CAP = EXPECTED_RESPONSES["Q04"] + REPAIR_ALLOWANCE["Q04"]


def validate_real_request_v14(
    *, model: str, response_cap: int, automatic_retries: int,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v14 real authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v14 real authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def execute_validation_v14(
    *, api_key: str, registry: Any, transport_factory: Any | None,
    ...
    return execute_validation(
        ...
        candidate_type=NativeToolOnlineCandidateCombinedV14,
        prompt_version=COMBINED_PROMPT_VERSION,
        schema_namespace="combined-prompt-v14-q04",
...
__all__ = [
    ...
    "execute_validation_v14", "validate_real_request_v14",
]
