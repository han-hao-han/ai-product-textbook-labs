"""Single-use Q04 real checkpoint for combined Prompt v11."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_combined_prompt_v11_q04_real_validation.json"
REAL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V11_Q04_REAL_CALLS"
QUESTION_ORDER = ("Q04",)
RESPONSE_CAP = sum(EXPECTED_RESPONSES[item] for item in QUESTION_ORDER)


def validate_real_request_v11(
    *, model: str, response_cap: int, automatic_retries: int,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v11 real authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v11 real authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def execute_validation_v11(
    *, api_key: str, registry: Any, transport_factory: Any | None,
    ...
    return execute_validation(
        ...
        candidate_type=NativeToolOnlineCandidateCombinedV11,
        prompt_version=COMBINED_PROMPT_VERSION,
        schema_namespace="combined-prompt-v11-q04",
...
__all__ = [
    ...
    "execute_validation_v11", "validate_real_request_v11",
]
