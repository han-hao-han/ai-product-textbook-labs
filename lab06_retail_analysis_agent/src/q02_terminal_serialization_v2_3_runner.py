"""Crash-safe single-Q02 runner for the V2.3 terminal transport."""
...
CONTRACT_PATH = (
    PROJECT_ROOT
    / "config"
    / "h3_q02_terminal_serialization_v2_3_real_validation.json"
)
DEFAULT_CONSUMPTION_ROOT = (
    PROJECT_ROOT / "results" / "raw" / "authorization_consumption"
)
...
REAL_CONFIRMATION = "I_AUTHORIZE_Q02_TERMINAL_SERIALIZATION_V2_3_REAL_CALLS"
OFFLINE_CONFIRMATION = "I_AUTHORIZE_Q02_TERMINAL_SERIALIZATION_V2_3_OFFLINE_ONLY"


class Q02TerminalV2_3RunnerError(ValueError):
    """Raised when the exact Q02 authority or protocol drifts."""


@dataclass(frozen=True)
class Q02ExecutionAuthorityV2_3:
...
def _load_contract() -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise Q02TerminalV2_3RunnerError("Q02 V2.3 contract must be an object")
    return value
...
def _validate_exact(
    *,
    ...
    if question_id != QUESTION_ID:
        raise Q02TerminalV2_3RunnerError("only Q02 is authorized")
    if model != MODEL:
        raise Q02TerminalV2_3RunnerError("model must be deepseek-v4-flash")
    if approved_model_responses != RESPONSE_CAP:
        raise Q02TerminalV2_3RunnerError("response cap must be exactly two")
    if automatic_retries != 0:
        raise Q02TerminalV2_3RunnerError("automatic retries must be zero")
...
def validate_real_execution_request(
    *,
    ...
    if confirmation != REAL_CONFIRMATION:
        raise Q02TerminalV2_3RunnerError("exact real confirmation is missing")
    ...
    if (
        contract.get("status")
        not in {
            "offline_runner_implementation_authorized_pending_validation",
            "offline_runner_validated_pending_real_execution",
        }
        ...
        raise Q02TerminalV2_3RunnerError(
            "Q02 V2.3 contract has no exact unconsumed authority"
        )
    if _consumption_path(authorization_id, consumption_root).exists():
        raise Q02TerminalV2_3RunnerError(
            "Q02 V2.3 real authority has already been consumed"
        )
    return Q02ExecutionAuthorityV2_3(
        mode="real_transport",
...
def validate_offline_execution_request(
    *,
    ...
    if confirmation != OFFLINE_CONFIRMATION:
        raise Q02TerminalV2_3RunnerError("offline confirmation is missing")
    return Q02ExecutionAuthorityV2_3(
        mode="offline_injected_transport",
        authorization_id="offline-not-a-real-authority",
    )
...
def consume_real_authority(
    ...
    *,
    ...
    if authority.mode != "real_transport":
        raise Q02TerminalV2_3RunnerError(
            "offline authority cannot create a real consumption record"
        )
...
def execute_q02_validation(
    *,
    ...
    candidate = NativeToolOnlineCandidateV2_3(**candidate_args)
    ...
    wire = candidate.transport_audit_payload()["requests"]
    failures: list[str] = []
    ...
    if [item["outbound_url"] for item in wire] != [
        "https://api.deepseek.com/beta/chat/completions",
        "https://api.deepseek.com/chat/completions",
    ]:
        failures.append("endpoint_sequence_mismatch")
    if [item["outbound_tool_count"] for item in wire] != [7, 0]:
        failures.append("terminal_tools_not_removed")
    if (
        wire[-1]["projected_tool_message_count"] != 1
        or wire[-1]["removed_tool_payload_keys"]
        != ("arguments", "result")
        or wire[-1]["dropped_assistant_tool_call_messages"] != 1
        or wire[-1]["outbound_tool_role_message_count"] != 0
        or wire[-1]["terminal_evidence_envelope_added"] is not True
    ):
        failures.append("terminal_evidence_projection_mismatch")

    passed = not failures
    case = {
        "schema_version": "1.5.6-h3-q02-terminal-serialization-v2.3-case-v1",
        "question_id": QUESTION_ID,
        "program_status": (
        ...
        "terminal_transport_audit": candidate.transport_audit_payload(),
    ...
    summary = {
        "schema_version": "1.5.6-h3-q02-terminal-serialization-v2.3-summary-v1",
        "run_id": actual_run_id,
        "execution_mode": authority.mode,
        "model": MODEL,
        ...
        "authorization_consumed": (
            ...
            and journal.state["authorization_consumed_when_attempt_reserved"]
        ),
