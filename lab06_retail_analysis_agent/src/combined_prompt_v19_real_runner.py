"""Q01/Q06 checkpoint and Q01-Q10 full validation for v19."""
...
from src.online_native_tool_candidate_combined_v19 import (
    COMBINED_PROMPT_VERSION, NativeToolOnlineCandidateCombinedV19,
)
...
CHECKPOINT_CONTRACT = PROJECT_ROOT / "config" / "h3_combined_prompt_v19_q01_q06_real_validation.json"
FULL_CONTRACT = PROJECT_ROOT / "config" / "h3_combined_prompt_v19_full_real_validation.json"
CHECKPOINT_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V19_Q01_Q06_REAL_CALLS"
FULL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V19_Q01_Q10_REAL_CALLS"
...
def _validate(contract: Path, exact_confirmation: str, exact_cap: int, *, model: str,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v19 authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v19 authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def validate_real_request_v19_checkpoint(**kwargs: Any) -> ToolRoutingAuthority:
    return _validate(CHECKPOINT_CONTRACT, CHECKPOINT_CONFIRMATION, CHECKPOINT_CAP, **kwargs)


def validate_real_request_v19_full(**kwargs: Any) -> ToolRoutingAuthority:
...
def _execute(order: tuple[str, ...], allowance: dict[str, int], namespace: str, **kwargs: Any) -> ToolRoutingRunResult:
    return execute_validation(
        candidate_type=NativeToolOnlineCandidateCombinedV19,
...
def execute_validation_v19_checkpoint(**kwargs: Any) -> ToolRoutingRunResult:
    return _execute(CHECKPOINT_ORDER, CHECKPOINT_ALLOWANCE, "combined-prompt-v19-q01-q06", **kwargs)


def execute_validation_v19_full(**kwargs: Any) -> ToolRoutingRunResult:
    return _execute(FULL_ORDER, FULL_ALLOWANCE, "combined-prompt-v19-full", **kwargs)


__all__ = [
    "CHECKPOINT_CAP", "FULL_CAP", "execute_validation_v19_checkpoint",
    "execute_validation_v19_full", "validate_real_request_v19_checkpoint",
    "validate_real_request_v19_full",
]
