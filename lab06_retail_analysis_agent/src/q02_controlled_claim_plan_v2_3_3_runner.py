"""Crash-safe single-Q02 runner for the V2.3.3 three-response boundary."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_q02_controlled_claim_plan_v2_3_3_real_checkpoint.candidate.json"
...
REAL_CONFIRMATION = "I_AUTHORIZE_Q02_CONTROLLED_CLAIM_PLAN_V2_3_3_REAL_CALLS"
OFFLINE_CONFIRMATION = "I_AUTHORIZE_Q02_CONTROLLED_CLAIM_PLAN_V2_3_3_OFFLINE_ONLY"


class Q02ControlledClaimPlanRunnerError(ValueError):
    pass


@dataclass(frozen=True)
class Q02AuthorityV2_3_3:
    mode: Literal["offline_injected_transport", "real_transport"]
    authorization_id: str


@dataclass(frozen=True)
class Q02ResultV2_3_3:
    passed: bool
...
def _contract() -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise Q02ControlledClaimPlanRunnerError("Q02 V2.3.3 contract must be an object")
    return value


def _validate_exact(question_id: str, model: str, responses: int, retries: int) -> None:
    if question_id != QUESTION_ID:
        raise Q02ControlledClaimPlanRunnerError("only Q02 is in scope")
    if model != MODEL:
        raise Q02ControlledClaimPlanRunnerError("model must be deepseek-v4-flash")
    if responses != RESPONSE_CAP:
        raise Q02ControlledClaimPlanRunnerError("response cap must be exactly three")
    if retries != 0:
        raise Q02ControlledClaimPlanRunnerError("automatic retries must be zero")


def validate_offline_execution_request(*, confirmation: str) -> Q02AuthorityV2_3_3:
    if confirmation != OFFLINE_CONFIRMATION:
        raise Q02ControlledClaimPlanRunnerError("offline confirmation is missing")
    return Q02AuthorityV2_3_3("offline_injected_transport", "offline-not-real")
...
def validate_real_execution_request(
    *, question_id: str, model: str, approved_model_responses: int,
    ...
    _validate_exact(question_id, model, approved_model_responses, automatic_retries)
    if confirmation != REAL_CONFIRMATION:
        raise Q02ControlledClaimPlanRunnerError("exact real confirmation is missing")
    value = _contract()
    ...
    if (
        ...
        or authority.get("authorized_by_user_current_turn") is not True
        ...
        raise Q02ControlledClaimPlanRunnerError("no exact unconsumed V2.3.3 real authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise Q02ControlledClaimPlanRunnerError("V2.3.3 real authority is already consumed")
    return Q02AuthorityV2_3_3("real_transport", authorization_id)


def _consume(authority: Q02AuthorityV2_3_3, root: Path) -> None:
    if authority.mode != "real_transport":
        raise Q02ControlledClaimPlanRunnerError("offline authority cannot be consumed")
    ...
    try:
        ...
        raise Q02ControlledClaimPlanRunnerError("V2.3.3 real authority is already consumed") from exc


def execute_q02(
    *, api_key: str, registry: Any, transport: Any | None,
    authority: Q02AuthorityV2_3_3, output_parent: Path,
    run_id: str | None = None,
    consumption_root: Path = DEFAULT_CONSUMPTION_ROOT,
) -> Q02ResultV2_3_3:
    if authority.mode == "offline_injected_transport" and transport is None:
        raise Q02ControlledClaimPlanRunnerError("offline execution requires injected transport")
    if authority.mode == "real_transport" and transport is not None:
        raise Q02ControlledClaimPlanRunnerError("real execution cannot inject transport")
    actual_run_id = run_id or datetime.now().astimezone().strftime(
        "q02_controlled_claim_plan_v2_3_3_%Y%m%dT%H%M%S_%f%z"
    )
    ...
    candidate = NativeToolOnlineCandidateV2_3_3(**args)
    ...
    plan_failed = outcome.error_stage == "claim_plan_validation"
    failures: list[str] = []
    ...
    if not plan_failed and (snapshot.attempted, snapshot.completed, snapshot.failed) != (3, 3, 0):
        failures.append("response_accounting_mismatch")
    if not plan_failed and audit.get("request_phases") != [
        "business_tool_selection", "controlled_claim_plan_generation",
        "final_report_from_validated_claim_plan",
    ]:
        failures.append("v2_3_3_phase_sequence_mismatch")
    passed = not failures
    root_stage = None if passed else (outcome.error_stage or "program_acceptance")
    root_codes = [] if passed else (
        ["invalid_controlled_claim_plan"] if plan_failed else failures[:1]
    )
    case = {
        "schema_version": "1.5.6-h3-q02-controlled-claim-plan-v2.3.3-case-v1",
        ...
        "claim_plan_trace": [item.__dict__ for item in candidate.claim_plan_trace],
    ...
    summary = {
        "schema_version": "1.5.6-h3-q02-controlled-claim-plan-v2.3.3-summary-v1",
        ...
        "final_report_request_sent": (
            candidate.claim_plan_trace[0].final_report_request_sent
            if candidate.claim_plan_trace else False
        ),
    ...
    return Q02ResultV2_3_3(passed, summary, output_dir)


__all__ = [
    "OFFLINE_CONFIRMATION", "REAL_CONFIRMATION", "Q02AuthorityV2_3_3",
    "Q02ControlledClaimPlanRunnerError", "execute_q02",
