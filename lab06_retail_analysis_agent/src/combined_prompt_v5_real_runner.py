"""Single-use four-question real checkpoint for combined Prompt v5."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_combined_prompt_v5_four_q_real_validation.json"
REAL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V5_FOUR_Q_REAL_CALLS"
QUESTION_ORDER = ("Q04", "Q06", "Q05", "Q07")
RESPONSE_CAP = sum(EXPECTED_RESPONSES[item] for item in QUESTION_ORDER)


def validate_real_request_v5(
    *, model: str, response_cap: int, automatic_retries: int,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v5 real authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v5 real authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def execute_validation_v5(
    *, api_key: str, registry: Any, transport_factory: Any | None,
    ...
    return execute_validation(
        ...
        candidate_type=NativeToolOnlineCandidateCombinedV5,
        prompt_version=COMBINED_PROMPT_VERSION,
        schema_namespace="combined-prompt-v5-four-q",
...
__all__ = [
    ...
    "execute_validation_v5", "validate_real_request_v5",
]
