"""Crash-safe Q02 runner for the V2.3.4.1 section-purpose checkpoint."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_q02_section_purpose_v2_3_4_1_real_checkpoint.candidate.json"
...
REAL_CONFIRMATION = "I_AUTHORIZE_Q02_SECTION_PURPOSE_V2_3_4_1_REAL_CALLS"
OFFLINE_CONFIRMATION = "I_AUTHORIZE_Q02_SECTION_PURPOSE_V2_3_4_1_OFFLINE_ONLY"


class Q02SectionPurposeRunnerError(ValueError):
    pass


@dataclass(frozen=True)
class Q02SectionPurposeAuthority:
    mode: Literal["offline_injected_transport", "real_transport"]
    authorization_id: str


@dataclass(frozen=True)
class Q02SectionPurposeResult:
    passed: bool
    summary: dict[str, Any]
    output_dir: Path


def validate_offline_execution_request(*, confirmation: str) -> Q02SectionPurposeAuthority:
    if confirmation != OFFLINE_CONFIRMATION:
        raise Q02SectionPurposeRunnerError("offline confirmation is missing")
    return Q02SectionPurposeAuthority("offline_injected_transport", "offline-not-real")
...
def validate_real_execution_request(
    *, question_id: str, model: str, approved_model_responses: int,
...
) -> Q02SectionPurposeAuthority:
    if (question_id, model, approved_model_responses, automatic_retries) != (
        QUESTION_ID, MODEL, RESPONSE_CAP, 0
    ):
        raise Q02SectionPurposeRunnerError("real request must be exact Q02/Flash/cap3/retry0")
    if confirmation != REAL_CONFIRMATION:
        raise Q02SectionPurposeRunnerError("exact real confirmation is missing")
    contract = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
    authority = contract.get("authorization", {})
    ...
    if (
        ...
        raise Q02SectionPurposeRunnerError("no exact unconsumed V2.3.4.1 authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise Q02SectionPurposeRunnerError("V2.3.4.1 authority is already consumed")
    return Q02SectionPurposeAuthority("real_transport", authorization_id)


def _consume(authority: Q02SectionPurposeAuthority, root: Path) -> None:
    ...
    try:
        ...
        raise Q02SectionPurposeRunnerError("V2.3.4.1 authority is already consumed") from exc


def execute_q02(
    *, api_key: str, registry: Any, transport: Any | None,
    authority: Q02SectionPurposeAuthority, output_parent: Path,
    run_id: str | None = None, consumption_root: Path = CONSUMPTION_ROOT,
) -> Q02SectionPurposeResult:
    if authority.mode == "offline_injected_transport" and transport is None:
        raise Q02SectionPurposeRunnerError("offline execution requires injected transport")
    if authority.mode == "real_transport" and transport is not None:
        raise Q02SectionPurposeRunnerError("real execution cannot inject transport")
    actual_run_id = run_id or datetime.now().astimezone().strftime(
        "q02_section_purpose_v2_3_4_1_%Y%m%dT%H%M%S_%f%z"
    )
    ...
    candidate = NativeToolOnlineCandidateV2_3_4_1(**args)
    ...
    planning_failed = outcome.error_stage == "atom_selection_validation"
    binding_failed = outcome.error_stage == "slot_report_binding_validation"
    failures: list[str] = []
    ...
    if not planning_failed and not binding_failed and (
        snapshot.attempted, snapshot.completed, snapshot.failed
    ) != (3, 3, 0):
        failures.append("response_accounting_mismatch")
    expected_phases = [
        "business_tool_selection", "section_purpose_atom_selection",
        "final_report_from_section_purpose_slots",
    ]
    if not planning_failed and audit.get("request_phases") != expected_phases:
        failures.append("v2_3_4_1_phase_sequence_mismatch")
    passed = not failures
    root_stage = None if passed else (outcome.error_stage or "program_acceptance")
    if planning_failed:
        root_codes = ["invalid_section_purpose_atom_selection"]
    elif binding_failed:
        root_codes = ["invalid_section_purpose_report_binding"]
    else:
        root_codes = [] if passed else failures[:1]
    case = {
        "schema_version": "1.5.6-h3-q02-section-purpose-v2.3.4.1-case-v1",
    ...
    summary = {
        "schema_version": "1.5.6-h3-q02-section-purpose-v2.3.4.1-summary-v1",
    ...
    return Q02SectionPurposeResult(passed, summary, output_dir)


__all__ = [
    "OFFLINE_CONFIRMATION", "REAL_CONFIRMATION", "Q02SectionPurposeAuthority",
    "Q02SectionPurposeRunnerError", "execute_q02",
