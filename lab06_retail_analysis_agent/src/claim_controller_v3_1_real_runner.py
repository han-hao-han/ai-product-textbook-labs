"""Q02 checkpoint and Q01-Q10 runner for V3.1."""
...
CHECKPOINT_CONTRACT = PROJECT_ROOT / "config" / "h3_claim_controller_v3_1_q02_real_validation.json"
FULL_CONTRACT = PROJECT_ROOT / "config" / "h3_claim_controller_v3_1_full_real_validation.json"
CHECKPOINT_CONFIRMATION = "I_AUTHORIZE_CLAIM_CONTROLLER_V3_1_Q02_REAL_CALLS"
FULL_CONFIRMATION = "I_AUTHORIZE_CLAIM_CONTROLLER_V3_1_Q01_Q10_REAL_CALLS"
CHECKPOINT_ORDER = ("Q02",)
FULL_ORDER = tuple(f"Q{index:02d}" for index in range(1, 11))
CHECKPOINT_ALLOWANCE = {"Q02": 1}
FULL_ALLOWANCE = {item: 1 if item <= "Q07" else 0 for item in FULL_ORDER}
CHECKPOINT_CAP = EXPECTED_RESPONSES["Q02"] + 1
...
def _validate(
    ...
    *,
    ...
    if confirmation != exact_confirmation:
        raise ToolRoutingRealRunnerError("exact V3.1 confirmation is missing")
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed V3.1 authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("V3.1 authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def validate_real_request_v3_1_checkpoint(**kwargs: Any) -> ToolRoutingAuthority:
    return _validate(CHECKPOINT_CONTRACT, CHECKPOINT_CONFIRMATION, CHECKPOINT_CAP, **kwargs)


def validate_real_request_v3_1_full(**kwargs: Any) -> ToolRoutingAuthority:
...
def _execute(
    ...
    return execute_validation(
        candidate_type=NativeToolOnlineCandidateClaimControllerV3_1,
...
def execute_validation_v3_1_checkpoint(**kwargs: Any) -> ToolRoutingRunResult:
    return _execute(CHECKPOINT_ORDER, CHECKPOINT_ALLOWANCE, "claim-controller-v3-1-q02", **kwargs)


def execute_validation_v3_1_full(**kwargs: Any) -> ToolRoutingRunResult:
    return _execute(FULL_ORDER, FULL_ALLOWANCE, "claim-controller-v3-1-full", **kwargs)


__all__ = [
    "CHECKPOINT_CAP", "CHECKPOINT_CONFIRMATION", "FULL_CAP", "FULL_CONFIRMATION",
    "execute_validation_v3_1_checkpoint", "execute_validation_v3_1_full",
    "validate_real_request_v3_1_checkpoint", "validate_real_request_v3_1_full",
]
