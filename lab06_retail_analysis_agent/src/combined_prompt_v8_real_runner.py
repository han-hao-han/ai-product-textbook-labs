"""Single-use Q04 real checkpoint for combined Prompt v8."""
...
CONTRACT_PATH = PROJECT_ROOT / "config" / "h3_combined_prompt_v8_q04_real_validation.json"
REAL_CONFIRMATION = "I_AUTHORIZE_COMBINED_PROMPT_V8_Q04_REAL_CALLS"
QUESTION_ORDER = ("Q04",)
RESPONSE_CAP = sum(EXPECTED_RESPONSES[item] for item in QUESTION_ORDER)


def validate_real_request_v8(
    *, model: str, response_cap: int, automatic_retries: int,
    ...
    if (
        ...
        raise ToolRoutingRealRunnerError("no exact unconsumed v8 real authority")
    if (consumption_root / f"{authorization_id}.json").exists():
        raise ToolRoutingRealRunnerError("v8 real authority is already consumed")
    return ToolRoutingAuthority("real_transport", authorization_id)


def execute_validation_v8(
    *, api_key: str, registry: Any, transport_factory: Any | None,
    ...
    return execute_validation(
        ...
        schema_namespace="combined-prompt-v8-q04",
...
__all__ = [
    ...
    "execute_validation_v8", "validate_real_request_v8",
]
